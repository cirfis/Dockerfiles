...
pacman_packages="git nginx php-fpm rsync openssl tmux mediainfo php-geoip zip libx264 libvpx xmlrpc-c sox python2 python-pip nodejs npm"

# update pacman
if [[ "$OS_ARCH" == "aarch64" ]];then
pacman -U http://mirror.archlinuxarm.org/aarch64/core/pacman-6.0.0-2-aarch64.pkg.tar.xz --noconfirm
else
pacman -U https://mirror.f4st.host/archlinux/core/os/x86_64/pacman-6.0.0-2-x86_64.pkg.tar.zst --noconfirm 
fi
rm /etc/pacman.conf /etc/pacman.conf.pacnew && mv -v /root/pacman.6.conf /etc/pacman.conf 

# install compiled packages using pacman
if [[ ! -z "${pacman_packages}" ]]; then
	pacman -Syyu --needed $pacman_packages --noconfirm
fi
...
# define aur packages
aur_packages="autodl-irssi-community flood-git"
...
# Fix root user error in install script
sed -i -e 's/^test -f.*in_docker=0$/in_docker=1/' update-to-head.sh
